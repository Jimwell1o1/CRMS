...
function createAdminUser($conn, $admin_name, $admin_email, $admin_uid, $admin_password){
    $sql = "INSERT INTO adminAcc (adminAccName, adminAccEmail, adminAccUid, adminAccPwd) VALUES (?, ?, ?, ?);";
    $stmt = mysqli_stmt_init($conn);
    if(!mysqli_stmt_prepare($stmt, $sql)){
    ...
    mysqli_stmt_bind_param($stmt, "ssss", $admin_name, $admin_email, $admin_uid, $hashedPwd);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
